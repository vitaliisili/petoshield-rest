...
class TestJobTicketEndpoints:
    ...
    def test_create_job_ticket_success(self, staff_user, api_client):
        api_client.force_authenticate(staff_user)
        data = {
            "position": "Engineer",
    ...
    def test_create_job_ticket_unauthorized(self, simple_user, api_client):
        api_client.force_authenticate(simple_user)
        data = {
            "position": "Engineer",
            "description": "Job description here",
    ...
    def test_get_job_ticket_success(self, staff_user, api_client, job_ticket):
        api_client.force_authenticate(staff_user)
        response = api_client.get(f'{self.endpoint}{job_ticket.id}/')
        assert response.status_code == 200
        assert json.loads(response.content).get('id') == job_ticket.id

    def test_update_job_ticket_success(self, staff_user, api_client, job_ticket):
        api_client.force_authenticate(staff_user)
        updated_data = {
    ...
    def test_delete_job_ticket_success(self, staff_user, api_client, job_ticket):
        api_client.force_authenticate(staff_user)
...
class TestPartnerTicketEndpoints:
    ...
    def test_create_partner_ticket_success(self, admin_user, api_client):
        api_client.force_authenticate(admin_user)
        data = {
            "business_name": "Britney Spears Corp",
            "contact_email": "contact@bscorp.com",
        }
        response = api_client.post(self.endpoint, data=data, format='json')
        assert response.status_code == 201
        assert 'id' in json.loads(response.content)

    def test_create_partner_ticket_unauthorized(self, simple_user, api_client):
        api_client.force_authenticate(simple_user)
        data = {
            "business_name": "Britney Spears Corp",
            "contact_email": "contact@bscorp.com",
    ...
    def test_get_partner_ticket_success(self, admin_user, api_client, partner_ticket):
        api_client.force_authenticate(admin_user)
        response = api_client.get(f'{self.endpoint}{partner_ticket.id}/')
        assert response.status_code == 200
        assert json.loads(response.content).get('id') == partner_ticket.id

    def test_update_partner_ticket_success(self, admin_user, api_client, partner_ticket):
        api_client.force_authenticate(admin_user)
        updated_data = {
            "business_name": "Updated Business",
            "contact_email": "updated@business.com",
        }
        response = api_client.put(f'{self.endpoint}{partner_ticket.id}/', data=updated_data, format='json')
        assert response.status_code == 200

    def test_delete_partner_ticket_success(self, admin_user, api_client, partner_ticket):
        api_client.force_authenticate(admin_user)
